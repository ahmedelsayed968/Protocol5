...
void Protocol::from_physical_layer(){
    ...
    // craete a fixed packet contains a data called ACK
    Packet empty_pk;
    empty_pk.data[0] = (unsigned char) 'A';
    ack_frame->setInfo(empty_pk);
...
//    event_type event;
//
//    this->enable_network_layer();			/* init */
...
Protocol::Protocol(Physical_Layer& physicalLayer) {
    this->common_physical_layer = &physicalLayer;
    this-> Id = Protocol::countP;
    Protocol::countP++;
    this->num_Ack = 1;
    this->num_seq = 1;
}
...
//    Frame * frame = Network_layer_packets.
    if(frame.getKind()==data){
        ...
        cout << "Message: " << frame.getInfo().data << endl;
